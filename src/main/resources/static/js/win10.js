...
window.Win10 = {
		...
		checkClick : function() {
			if (document.activeElement) {
				...
				for ( var i in this.iframes) {
					var eid = undefined;
					if ((eid = this.iframes[i].element.id)
							&& !document.getElementById(eid)) {
						delete this.iframes[i];
						continue;
					}
					if (activeElement === this.iframes[i].element) { // user
																		// is in
																		// this
																		// Iframe
	...
	_renderBar : function() {
		...
		btns
				.css('width',
						('calc(' + (1 / this._countTask * 100) + '% - 1px )'))
	},
	_handleReady : [],
		...
		if (Win10.isSmallScreen()) {
			if (Win10._bgs.mobile) {
				loaders.each(function() {
					var loader = $(this);
					if (loader.attr('src') === Win10._bgs.mobile
							&& loader.hasClass('loaded')) {
				...
				if (!flag) {
					// 没找到加载完毕的图片
					var img = $('<img class="img-loader" src="'
							+ Win10._bgs.mobile + '" />');
					$('#win10').append(img);
			...
			if (Win10._bgs.main) {
				loaders.each(function() {
					var loader = $(this);
					if (loader.attr('src') === Win10._bgs.main
							&& loader.hasClass('loaded')) {
	...
	_startAnimate : function() {
		setInterval(function() {
			...
			if (animated_liveness === 0 || classes_lenth === 0
					|| !$("#win10-menu").hasClass('opened')) {
				return;
			}
			$('#win10-menu>.blocks>.menu_group>.block').each(
					function() {
						if (!$(this).hasClass('onAnimate')
								&& Math.random() <= animated_liveness) {
							var that = $(this);
							var class_animate = Win10._animated_classes[Math
									.floor((Math.random() * classes_lenth))];
	...
	_fixWindowsHeightAndWidth : function() {
		...
		opens.each(function() {
			...
			if (height + 40 >= clientHeight) {
				...
				layero_opened.find('.layui-layer-content').css('height',
						clientHeight - 83);
	...
	_bind_open_windows : function() {
		...
		$('#win10').on(
				...
				function() {
					...
					if ($this.data('url') !== "") {
						// >> 获取弹窗标题
						var title = $this.data('title') || '', areaAndOffset;
						// >> 判断是否有标题图片
						var bg = $this.data('icon-bg') ? $this.data('icon-bg')
								: '';
						if ($this.data('icon-image')) {
							// >> 加入到标题中
							title = '<img class="icon ' + bg + '" src="'
									+ $this.data('icon-image') + '"/>' + title;
						}
						if ($this.data('icon-font')) {
							// >> 加入到标题中
							title = '<i class="fa fa-fw fa-'
									+ $this.data('icon-font') + ' icon ' + bg
									+ '"></i>' + title;
						}
						if (!title && $this.children('.icon').length === 1
								&& $this.children('.title').length === 1) {
							title = $this.children('.icon').prop("outerHTML")
	...
	_init : function() {
		...
		// 获取语言
		this._lang = (navigator.language || navigator.browserLanguage)
				.toLowerCase();
				...
				.on(
						...
						function() {
							...
							if (e.hasClass('has-sub-down')) {
								$('#win10-menu .list .item.has-sub-up')
										.toggleClass('has-sub-down')
										.toggleClass('has-sub-up');
								$("#win10-menu .list .sub-item").slideUp();
							}
							if (e.next().hasClass('sub-item')) {
								e.toggleClass('has-sub-down').toggleClass(
										'has-sub-up');
							}
		...
		setInterval(function() {
			...
			$("#win10_btn_time").html(
					hours + ':' + mins + '<br/>' + year + '/' + month + '/'
							+ date);
		}, 1000);
		// 离开前警告
		document.body.onbeforeunload = function(event) {
			var rel = Win10.lang('系统可能不会保存您所做的更改',
					'The system may not save the changes you have made.');
			if (!window.event) {
		...
		$(window).resize(function() {
			...
			Win10._checkBgUrls();
			if (!Win10.isSmallScreen())
				Win10._fixWindowsHeightAndWidth(); // 2017年11月14日修改，加入了if条件
		});
		// 细节
		$(document).on('focus', ".win10-layer-open-browser textarea",
				function() {
		...
		$(document).on(
				...
				function(e) {
					if (e.keyCode === 13) {
						$(this).parent().parent().find('.layui-layer-btn0')
								.click();
					}
				});
		// 点击清空右键菜单
		...
		Win10
				.setContextMenu(
						...
						[
								[
										'<i class="fa fa-fw fa-star"></i> 收藏本页',
										function() {
		...
		Win10.setContextMenu('#win10_btn_group_middle', [
				[
						'<i class="fa fa-fw fa-window-maximize"></i> '
								+ Win10.lang('全部显示', 'Show All Windows'),
						function() {
							Win10.showWins()
						} ],
				[
						'<i class="fa fa-fw fa-window-minimize"></i> '
								+ Win10.lang('全部隐藏', 'Hide All Windows'),
						function() {
							Win10.hideWins()
						} ],
				[
						'<i class="fa fa-fw fa-window-close"></i> '
								+ Win10.lang('全部关闭', 'Close All Windows'),
						function() {
							Win10.closeAll()
						} ], ]);
		...
		/**
		 * WIN10-UI v1.1.2.2 桌面舞台支持补丁 WIN10-UI v1.1.2.2之后的版本不需要此补丁
		 * 
	...
	newMsg : function(title, content, handle_click) {
		var e = $('<div class="msg">' + '<div class="title">' + title
				+ '</div>' + '<div class="content">' + content + '</div>'
				+ '<span class="btn_close_msg fa fa-close"></span>' + '</div>');
		...
		layer.tips(Win10.lang('新消息:', 'New message:') + title,
				'#win10_btn_command', {
					tips : [ 1, '#3c6a4a' ],
					time : 3000
				});
	...
	openUrl : function(url, title, areaAndOffset) {
		...
		if (this.isSmallScreen() || areaAndOffset === 'max') {
			...
			offset = [
					Math.round((this._countTask % 10 * 20) + topset) + 'px',
					Math.round((this._countTask % 10 * 20 + 100) + leftset)
							+ 'px' ];
		}
		...
		var btn = $('<div id="win10_' + index + '" index="' + index
		...
		layero_opened
				...
				.prepend(
						'<a class="win10-btn-change-url" index="'
								+ index
								+ '" title="'
								+ Win10.lang('修改地址', 'Change URL')
								+ '" href="javascript:void(0)"><span class="fa fa-chain"></span></a><a class="win10-btn-refresh" index="'
								+ index
								+ '" title="'
				...
				function() {
					setTimeout(function() {
						...
						if (height >= document.body.clientHeight) {
							...
							layero_opened.find('.layui-layer-content').css(
									'height', height - 83);
							layero_opened.find('.layui-layer-content iframe')
									.css('height', height - 83);
						}
					}, 300);
		...
		btn.click(function() {
			...
			if ($(this).hasClass('show')) {
				if ($(this).hasClass('active')) {
					...
					$('#win10_btn_group_middle .btn.active').removeClass(
							'active');
		...
		Win10._iframeOnClick
				.track(
						layero_opened.find('iframe:first')[0],
						function() {
							if (Object
									.getOwnPropertyNames(Win10._iframe_click_lock_children).length === 0) {
								Win10._settop(layero_opened);
								Win10._checkTop();
							} else {
	...
	exit : function() {
		layer.confirm(Win10.lang('确认要关闭本页吗?',
			...
			window.close();
			layer.close(index);
			layer.alert(Win10.lang('哎呀,好像失败了呢。',
